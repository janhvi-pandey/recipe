  ...
  return (
    <div className=" text-black min-h-screen">
      ...
      {loading ? (
        ...
        <>
          {/* Hero Section */}
          <div className=" flex items-center justify-start p-8 md:p-16 h-[300px] md:h-[355px] bg-[url('/images/image3.png')] bg-cover bg-center bg-no-repeat">
            <div className=" inset-0  bg-opacity-50"></div>
            <h1 className=" text-5xl font-bold pt-24 text-black">{recipe.name}</h1>
          </div>

          {/* Recipe Content */}
          <div className="max-w-5xl mx-auto px-6 py-10">
            <div className="flex flex-col md:flex-row items-center justify-between gap-10">
              <img className="rounded-3xl w:[300px] md:w-[400px] h-[300px] md:h-[400px] shadow-lg" src={recipe.image} alt={recipe.name} />
            ...
            {/* Ingredients Section */}
            <div className="mt-10">
              <h2 className="text-3xl font-semibold  text-amber-900">Ingredients</h2>
            ...
            <div className="mt-10">
              <h2 className="text-3xl font-semibold text-amber-900">Instructions</h2>
              <ul className="mt-4 space-y-3 text-lg">
                {recipe.instructions.map((instruction, index) => (
                  <li key={index} className="p-3 bg-gray-200 rounded-lg shadow-md">
                    <span className="font-semibold text-[20px]">Step {index + 1}: </span>
                    {instruction}
                  </li>
                ))}
              </ul>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mt-10">
              {[
                ...
                <div key={index} className="flex items-center p-4 bg-gray-200 rounded-lg shadow-md">
                  <div className="w-12 h-12 flex items-center justify-center bg-amber-700 text-white rounded-lg">
                    {item.icon}
                  </div>
                  <div className="ml-4">
        ...
        <p className="text-center py-10 text-lg text-gray-700">Recipe not found.</p>
      )}
